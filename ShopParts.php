...
// Fetch categories from the database where associated products have Type 'Parts'
$categories_sql = "SELECT DISTINCT pc.* FROM `Product category` pc
                    INNER JOIN `Products` p ON pc.CategoryID = p.CategoryID
                    WHERE p.Type = 'Parts' AND p.Visibility = 'Yes'";
...
// Fetch parts from the database with the selected category and inStock filters
$sql = "SELECT * FROM `products` p WHERE p.Type = 'Parts' AND p.Visibility = 'Yes' ".$category_filter." ".$in_stock_filter;
...
$sql = "SELECT * FROM `products` p WHERE p.Type = 'Parts' AND p.Visibility = 'Yes' ".$category_filter." ".$in_stock_filter." ".$search_filter;
$result = $conn->query($sql);
?>
...
<!--TopSection: navigation for the shop area-->
<div class="container-fluid TopSection">
    <div class="row filter">
        <div class="col-5 col-md-2">
            <div class="form-floating">
                <!--To allow users to look at products in categories-->
                <select class="form-select" id="floatingSelect" aria-label="Floating label select"
                        onchange="location = this.value;">
                    <option value="ShopParts.php?category=0" <?php if(isset($_GET['category']) && $_GET['category'] == '0') echo 'selected';?>>None</option>
                    <?php
                    if ($categories_result->num_rows > 0) {
                        while ($category = $categories_result->fetch_assoc()) {
                            echo '<option value="ShopParts.php?category='.$category['CategoryID'].'" '.(isset($_GET['category']) && $_GET['category'] == $category['CategoryID'] ? 'selected':'').'>' . $category['catName'] . '</option>';
                        }
                    }
                    ?>
                </select>
        ...
        <div class="col-md-2 d-none d-md-block">
            <!--Allow users to see what is in stock-->
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="1" id="flexCheckCheckedLarge" <?php if(isset($_GET['inStock']) && $_GET['inStock'] == '1') echo 'checked';?> onchange="location.href = 'ShopParts.php?category=<?php echo isset($_GET['category']) ? $_GET['category'] : '0'; ?>&inStock=' + (this.checked ? '1' : '0');">
        ...
        <div class="col-5 col-md-2">
            <!--Allows users to seach for products-->
    ...
    <div class="row d-md-none">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" value="1" id="flexCheckCheckedSmall" <?php if(isset($_GET['inStock']) && $_GET['inStock'] == '1') echo 'checked';?> onchange="location.href = 'ShopParts.php?category=<?php echo isset($_GET['category']) ? $_GET['category'] : '0'; ?>&inStock=' + (this.checked ? '1' : '0');">
...
<div class="container-fluid">
    <div class="row">
        <?php
        // Check if any parts are found
        if ($result->num_rows > 0) {
            // Output data of each part
            while ($row = $result->fetch_assoc()) {
                ?>
                <div class="col-md-4">
                    <div class="card">
                        <!--Cards used for product information-->
