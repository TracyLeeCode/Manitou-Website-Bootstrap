<?php
// Establish a connection to your database (if not already done)
...
// Fetch categories from the database where associated products have Type 'Machine' and Visibility must equal Yes
$categories_sql = "SELECT DISTINCT pc.* FROM `Product category` pc
                    INNER JOIN `Products` p ON pc.CategoryID = p.CategoryID
                    WHERE p.Type = 'Machine' AND p.Visibility = 'Yes'";
...
// Fetch machines from the database with the selected category and inStock filters
$sql = "SELECT * FROM `products` p WHERE p.Type = 'Machine' AND p.Visibility = 'Yes' ".$category_filter." ".$in_stock_filter;

/*echo "SQL Query: " . $sql; // Debugging line to print out the SQL query*/
...
$sql = "SELECT * FROM `products` p WHERE p.Type = 'Machine' AND p.Visibility = 'Yes' ".$category_filter." ".$in_stock_filter." ".$search_filter;
$result = $conn->query($sql);
?>
...
<!--TopSection: navigation for the shopping area-->
<div class="container-fluid TopSection">
    <div class="row filter">
        <div class="col-5 col-md-2">
            <div class="form-floating">
                ...
                <select class="form-select" id="floatingSelect" aria-label="Floating label select"
                    ...
                    <option value="ShopMachines.php?category=0" <?php if(isset($_GET['category']) && $_GET['category'] == '0') echo 'selected';?>>None</option>
                    <?php
                    if ($categories_result->num_rows > 0) {
                        while ($category = $categories_result->fetch_assoc()) {
                            echo '<option value="ShopMachines.php?category='.$category['CategoryID'].'" '.(isset($_GET['category']) && $_GET['category'] == $category['CategoryID'] ? 'selected':'').'>' . $category['catName'] . '</option>';
                        }
                    }
                    ?>
                </select>
        ...
        <!--In stock check box to show items that are in stock-->
        <div class="col-md-2 d-none d-md-block">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="1" id="flexCheckCheckedLarge" <?php if(isset($_GET['inStock']) && $_GET['inStock'] == '1') echo 'checked';?> onchange="location.href = 'ShopMachines.php?category=<?php echo isset($_GET['category']) ? $_GET['category'] : '0'; ?>&inStock=' + (this.checked ? '1' : '0');">
        ...
        <div class="col-5 col-md-2">
            <!--Search functionality to allow users to search products-->
    ...
    <div class="row d-md-none">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" value="1" id="flexCheckCheckedSmall" <?php if(isset($_GET['inStock']) && $_GET['inStock'] == '1') echo 'checked';?> onchange="location.href = 'ShopMachines.php?category=<?php echo isset($_GET['category']) ? $_GET['category'] : '0'; ?>&inStock=' + (this.checked ? '1' : '0');">
...
<div class="container-fluid">
    <div class="row">
        <?php
        // Check if any machines are found
        if ($result->num_rows > 0) {
            // Output data of each machine
            while ($row = $result->fetch_assoc()) {
                ?>
                <div class="col-md-4">
                    <!--Cards for products information-->
                    <div class="card">
